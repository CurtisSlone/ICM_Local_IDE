...
$inst = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "windows-icm"
$name = "__smoke__"
$pass = 0; $fail = 0
...
Push-Location $inst
try {
    if (Test-Path (Join-Path "out" $name)) { Remove-Item -Recurse -Force (Join-Path "out" $name) }

    # 1. scaffold
    $o = Tool "new_project.ps1" @("$name console")
    Check "new_project reports created" ($o -match "OK: created") $o
    Check "project.json exists" (Test-Path "out\$name\project.json") ""
    Check "response.rsp exists" (Test-Path "out\$name\response.rsp") ""

    # 2. project.json is a real JSON OBJECT with name (catches the [string]-param coercion bug)
    $meta = Get-Content -Raw "out\$name\project.json" | ConvertFrom-Json
    ...
    Check "dist exe produced" (Test-Path "out\$name\dist\$name.exe") ""
    ...
    Check "staged file on disk" (Test-Path "out\$name\src\Core\Thing.cs") ""
    ...
    $meta2 = Get-Content -Raw "out\$name\project.json" | ConvertFrom-Json
    ...
    Remove-Item -Recurse -Force (Join-Path "out" $name)
}
